...
function AdminModules() {
    ...
    const confirmDelete = async () => {
        try {
            ...
            const response = await fetch(`http://localhost:4000/api/courses/${moduleToDelete.id}`, {
                method: 'DELETE',
                headers: {
            ...
            if (response.ok) {
                setCourses(prevCourses => prevCourses.filter(course => course.id !== moduleToDelete.id));
            } else {
    ...
    const navItems = [
        { text: "Users", icon: <Home size={20} />, route: "/AdminDashboard" },
        { text: "Courses", icon: <Book size={20} />, route: "/AdminModules" },
        { text: "Notifications", icon: <Bell size={20} />, route: "/AdminNotifications" },
        { text: "Announcements", icon: <FileText size={20} />, route: "/AdminAnnouncements" },
    ];
            ...
            <div className="flex h-screen bg-gray-100">
                ...
                <div className="flex-1 p-6 overflow-auto">
                                    ...
                                    >
                                     ...
                                     <label className="block text-sm font-medium text-gray-700 mt-4"> Image </label>
                                        <input type="file" name="image" onChange={handleImageChange} className="mt-1 block w-full" accept="image/*"/>
                                           {editingCourse.image && (
                                                <img src={editingCourse.image} alt="Course" className="mt-2 h-20 w-auto" />
                                            )}
                                   <label className="block text-sm font-medium text-gray-700 mt-4">
                                        Learner Group
                                    </label>
                                    ...
                                    <select  name="student_teacher_group_id"
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                  ...
                                  <div>
                                     <p className="font-semibold">Teacher</p>
                                        <p className="text-gray-700">{course.teacher ? `${course.teacher.first_name} ${course.teacher.last_name}` : 'Not assigned'}</p>

                                         <p className="font-semibold mt-2">Learner Group</p>
                                          <p className="text-gray-700">{course.learnerGroup ? course.learnerGroup.name : 'Not assigned'}</p>
                                        <p className="font-semibold mt-2">Student Teacher Group</p>
                                          <p className="text-gray-700">{course.studentTeacherGroup ? course.studentTeacherGroup.name : 'Not assigned'}</p>

                                  </div>
                                </div>
                                 <div className="mt-4">
                                        {course.image && (
                                        <img src={course.image} alt={course.name} className="h-48 w-auto" />
                                            )}
                                    </div>
                                </div>
                          )}
                  ...
                  <li className="p-4 text-center text-gray-600">
                    ...
                    {/* Add Course Modal */}
                    {isAddCourseOpen && (
                        <Modal isOpen={isAddCourseOpen} onClose={() => setIsAddCourseOpen(false)}>
