...
const CreateGroupModal = ({ onClose, onSave }) => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "type") {
      // Clear members when group type changes
      setGroupData((prev) => ({
        ...prev,
        [name]: value,
        members: [], // Clear members array
      }));

      // Clear available members to force reload with new type
      setAvailableMembers([]);
    } else {
      setGroupData((prev) => ({
        ...prev,
  ...
  const addMember = (member) => {
    // Remove from available members and add to selected members
  ...
  const removeMember = (member) => {
    // Add back to available members and remove from selected members
